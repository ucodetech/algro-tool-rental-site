...
{
  $toold = $this->_db->get('farmTools', array('available', '=', 1));
  if ($toold->count()) {
      return   $toold->results();
  }else {
    return '<h2 class="text-center text-danger">No Farm Tool at the moment!</h2>';
  }
}


public function totalCountPayment()
{
  $money =  $this->_db->query("SELECT SUM(item_price) AS total FROM transactions");
   if ($money->count()) {
       return $money->results();
   }else{
    return '0';
   }
}
