...
public function fetchNotifactionCount(){
  ...
  public function fetchNotifactionAdmin(){
    $sql = "SELECT * FROM notification WHERE type = 'admin' AND deleted =  0 ORDER BY id DESC";
     $this->_db->query($sql);
    if ($this->_db->count()) {
  ...
  public function fetchNotifactionCountAdmin(){
    $sql = "SELECT * FROM notification WHERE deleted = 0 ";
     $this->_db->query($sql);
      $output = '';
      if ($this->_db->count()) {
          $count = $this->_db->count();
  ...
  public function fetchNotifactionAdminTitle(){
  ...
  //Delete notification
    public function removeNotificationAdmin($id){
      $sql = "UPDATE notification SET deleted = 1 WHERE id = '$id' AND type = 'admin'";
        $this->_db->query($sql);
      return true;
    }
